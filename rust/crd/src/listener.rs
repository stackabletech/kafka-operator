use crate::{
    ...
    SECURE_CLIENT_PORT_NAME, SECURE_INTERNAL_PORT, STACKABLE_LISTENER_DIR,
};
...
pub fn get_kafka_listener_config(
    ...
    if kafka.client_authentication_class().is_some() {
        ...
        advertised_listeners.push(KafkaListener {
            ...
            port: node_port_cmd(STACKABLE_LISTENER_DIR, SECURE_CLIENT_PORT_NAME),
        });
        ...
        advertised_listeners.push(KafkaListener {
            name: KafkaListenerName::Client,
            host: node_cmd(STACKABLE_LISTENER_DIR),
            port: node_port_cmd(STACKABLE_LISTENER_DIR, CLIENT_PORT_NAME),
        });
        listener_security_protocol_map
