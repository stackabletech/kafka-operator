...
// Builds two Kafka listeners: CLIENT and INTERNAL.
// The INTERNAL listener binds *and* advertises the corresponding pod of the headless service.
// The CLIENT listener binds to 0.0.0.0 and advertises the corresponding "broker listener" service.
// Every broker in the cluster has its own broker listener service created by the listener operator.
// TODO: This function is tightly coupled with `crd::security::KafkaTlsSecurity::broker_listener_tls_properties`.
pub fn get_kafka_listener_config(
    kafka: &v1alpha1::KafkaCluster,
    kafka_security: &KafkaTlsSecurity,
    _rolegroup_ref: &RoleGroupRef<v1alpha1::KafkaCluster>,
    cluster_info: &KubernetesClusterInfo,
) -> Result<KafkaListenerConfig, KafkaListenerError> {
    let pod_fqdn = internal_pod_fqdn(kafka, cluster_info)?;
    ...
    listener_security_protocol_map
        .insert(KafkaListenerName::Client, kafka_security.client_protocol());

    // INTERNAL (advertised) listener
    listeners.push(KafkaListener {
    ...
    listener_security_protocol_map.insert(
        ...
        kafka_security.internal_protocol(),
    );
...
pub fn internal_pod_fqdn(
    kafka: &v1alpha1::KafkaCluster,
    cluster_info: &KubernetesClusterInfo,
) -> Result<String, KafkaListenerError> {
    Ok(format!(
        "${{env:POD_NAME}}.${{env:ROLEGROUP_HEADLESS_SERVICE_NAME}}.{namespace}.svc.{cluster_domain}",
    ...
    fn test_get_kafka_listeners_config() {
        ...
        assert_eq!(
            ...
            format!(
                ...
                internal_host = internal_pod_fqdn(&kafka, &cluster_info).unwrap(),
