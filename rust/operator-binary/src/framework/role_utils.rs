...
//! `smooth-operator` branch, with simplifications appropriate for kafka-operator.
//!
...
//!   implement `Merge`. Kafka uses `JavaCommonConfig`, which intentionally does not
...
/// Kafka-friendly view of a validated, merged `RoleGroup`.
