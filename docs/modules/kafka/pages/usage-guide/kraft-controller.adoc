...
* Each controller pod runs an additional `quorum-manager` sidecar container. On startup it admits the pod into
  the KRaft voter set (`kafka-metadata-quorum.sh add-controller`), and on pod termination (`preStop`) it removes
   the pod from the voter set again (`remove-controller`).
...
* Admitting a controller into the KRaft voter set is *solely* the concern of the `quorum-manager` sidecar container
  — the format step never asserts a voter list. Exactly one controller (the one with the numerically
...
* `controller.quorum.bootstrap.servers` (used by the `kafka` process itself to find the controller quorum, and by
  ...
  role group's own headless Service DNS name, not individual pod addresses.
...
removes the pod from the KRaft voter set as described under "Internal operator details" above.
