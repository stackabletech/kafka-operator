...
* The single controller chosen to bootstrap the quorum standalone is picked by a stable, deterministic rule
  ...
  the existing one. If this happens, recovery requires manual intervention, see <<Controller lost its persistent volume>>.
* A Controller that loses its persistent volume is not re-admitted to the voter set automatically, because it
  ...
  See <<Controller lost its persistent volume>>.
...
=== Controller lost its persistent volume

A KRaft voter is identified by the pair (`node.id`, directory ID).
The `node.id` is derived deterministically by the operator and is stable for a given pod, but the directory ID is
generated by `kafka-storage.sh format` and stored in `meta.properties` on the `logDirs` volume.

If a Controller's PersistentVolume is lost -- an unrecoverable node failure with local storage, an accidentally
deleted PVC, a storage backend failure -- the pod comes back with an empty volume, formats again and therefore
receives a *new* directory ID.
The quorum, meanwhile, still lists the *old* directory ID for that same `node.id`, because nothing removed it:
the `preStop` hook only runs on graceful termination, and a lost volume usually means there was none.

==== Symptoms

The Controller pod starts, but never becomes ready:

* The `quorum-manager` sidecar logs `Local Raft state is observer, attempting add-controller...` in a loop, and
  every attempt fails with `DuplicateVoterException`:
+
[source,text]
----
org.apache.kafka.common.errors.DuplicateVoterException: The voter id for ReplicaKey(id=2110489705,
directoryId=Optional[dGZVBqJEz4LOwpIHPLM6sw]) is already part of the set of voters [...]
----
+
The first directory ID in that message is the pod's new one, and the entry for the same `id` in the listed voter
set carries the stale one.
* The readiness probe keeps failing, because the local Raft state stays `observer` instead of reaching `leader`,
  `follower` or `voted`.

Because Controllers use `OrderedReady` Pod management, a Controller stuck in this state also blocks any
subsequent rolling restart or scale-up of the role group.

==== Recovery

The stale voter entry has to be removed from the quorum by hand, after which the sidecar's `add-controller` loop
admits the pod under its new directory ID on its own.

Run the following from a *healthy* Controller pod. First, look up the voter set:

[source,bash]
----
$ BOOTSTRAP=$(grep '^controller.quorum.bootstrap.servers=' /stackable/config/controller.properties | cut -d= -f2- | sed 's/\\:/:/g')
...
The affected `node.id` appears *twice*: once as a `Leader` or `Follower` -- that is the stale voter entry, and
the directory ID to remove -- and once as an `Observer`, which is the recovered pod fetching from the quorum.
Take the directory ID from the voter row, then remove it:
...
--bootstrap-controller "$BOOTSTRAP" \
--command-config /stackable/config/admin-client.properties \
remove-controller --controller-id <NODE_ID> --controller-directory-id <STALE_DIRECTORY_ID>
----

WARNING: Make sure the directory ID you pass is the stale one, taken from the `describe --replication` output,
and not the one in the recovered pod's `meta.properties`.
Removing the entry of a healthy Controller shrinks the voter set and reduces the number of failures the quorum
can tolerate.

The affected pod joins the voter set within one iteration of the sidecar loop (10 seconds) and becomes ready
shortly after.
The removed entry stays visible in `describe --replication` as an `Observer` row for a while; that is harmless.

==== If the affected Controller is the bootstrap Controller

The Controller with the numerically lowest `node.id` is the one the operator formats with
`kafka-storage.sh format --standalone`, and this rule is only correct for a cluster's original bootstrap.
If *that* Controller loses its volume while the rest of the cluster is running, it does not rejoin the existing
quorum: it formats a second, conflicting single-node quorum with an empty metadata log.

Do not let it start with an empty volume.
Stop the cluster before the pod is recreated, by setting the following on the `KafkaCluster` resource:
...
Then format the replacement volume out of band -- mounting the PVC into a throwaway Pod that runs
`kafka-storage.sh format` with the cluster's `cluster-id` and `--no-initial-controllers` -- before starting the
cluster again.
The operator's own format call uses `--ignore-formatted` and therefore skips an already formatted volume, so the
`--standalone` flag it passes has no effect on the pre-formatted volume.

NOTE: Recovering the bootstrap Controller is the one case that cannot be resolved with `remove-controller` alone.
If the cluster has already lost quorum as well, recovery is a manual KRaft operation and outside the scope of the
operator; refer to the Apache Kafka documentation.
