...
* `controller.quorum.bootstrap.servers` lists the individual pod FQDNs of all controllers.
  ...
  It means that, with Kerberos enabled, whenever the controller replicas is changed,
  **both** the controller **and** broker pods are rolled by the restart controller.
