...
* Admitting a controller into the KRaft voter set is *solely* the concern of the `quorum-manager` sidecar container
  ...
  the current replica count, so changing `spec.controllers.roleGroups.<name>.replicas` no longer forces Kubernetes to
  roll every already-existing controller pod — only the pods actually being added or removed are touched.
...
* Scaling controller replicas up or down today still restarts every existing controller pod, not just the ones
  being added or removed. This is not caused by the container command (which is confirmed live to stay identical
  across replica-count changes) but by the Stackable platform's ConfigMap-restarter mechanism: `controller.properties`
  still lists every current peer in `controller.quorum.bootstrap.servers`, so its `resourceVersion` genuinely
  changes on every replica-count change, and the platform restarts every pod referencing a changed ConfigMap
  (most products need a full restart to pick up config changes; the `quorum-manager` sidecar only reads this value
  once at its own startup, so for it specifically the restart isn't needed, but the platform mechanism can't tell
  the difference). Eliminating this would mean making that ConfigMap value itself stable across replica-count
  changes (for example, pointing at a fixed anchor node instead of the full peer list) — not yet done.
