...
We start by creating a dedicated namespace to work on and deploy the Kafka cluster including ZooKeeper and credentials.
...
2. Update the `KafkaCluster` resource to add Controller role group.
...
If the ZooKeeper cluster is also serving other usecases than Kafka you can skip this step.
