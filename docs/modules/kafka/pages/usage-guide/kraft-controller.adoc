...
* Each controller pod runs an additional `quorum-manager` sidecar container that, on startup, admits the pod into
  ...
  *own* `preStop` hook instead, not the sidecar's: `preStop` only delays that same container's `SIGTERM`, and it's
  the `kafka` container's own Raft process — the thing actually leaving the voter set — that needs to stay alive
  while removal is attempted, which matters most when the departing pod is the current leader.
