...
* `controller.quorum.bootstrap.servers` lists the individual pod FQDNs of all controllers.
  This is required by Kerberos: a GSSAPI client derives the service principal from the hostname
  it dials, and the CONTROLLER listener can only offer the pod's own principal.
  It means that, with Kerberos enabled, the property changes whenever a controller role group is scaled,
  so the controller pods are rolled by the restart controller on scale.
