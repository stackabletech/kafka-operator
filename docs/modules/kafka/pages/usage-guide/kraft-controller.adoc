...
xref:usage-guide/security.adoc#_kerberos[Security -- Kerberos]; it applies to the whole
`KafkaCluster`, controllers included. As always with Kerberos, server and internal TLS are
required.
...
* Controller keytabs are *pod-scoped*, unlike broker keytabs, which are scoped to their
  listener volumes. Controllers are not exposed through a listener-operator `Listener`; they
  are reachable only under their own StatefulSet pod DNS name, which is therefore the
...
* The `quorum-manager` sidecar and the `preStop` hook authenticate to the CONTROLLER listener
  as the controller's own pod principal, so *dynamic quorum scaling works with Kerberos
  enabled*. Scaling a controller role group up or down needs no manual intervention.
