  ...
  brokers:
    roleGroups:
      default:
        replicas: 3
----
<1> The `tls.serverSecretClass` refers to the client-to-server encryption. Defaults to the `tls` secret. It can be deactivated by setting `config.tls` to `null`.
<2> The `tls.internalSecretClass` refers to the broker-to-broker internal encryption. This must be explicitly set or defaults to `tls`. Can be disabled by setting `config.internalTls` to `null`.
