...
----
sudo apt-get update
sudo apt-get install stackable-kafka-operator
----
...
[source,bash]
----
sudo yum install stackable-kafka-operator
----
...
[source,bash]
----
docker run -e KAFKA_OPERATOR_LOG=trace -e KUBECONFIG=/root/.kube/k3s.yaml --mount type=bind,source="$HOME/.kube/k3s.yaml",target="/root/.kube/k3s.yaml" -it docker.stackable.tech/stackable/kafka-operator:latest
----
