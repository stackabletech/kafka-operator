...
VERSION := $(shell cargo metadata --format-version 1 | jq -r '.packages[] | select(.name=="stackable-kafka-operator") | .version')
...
version:
	yq eval -i '.version = strenv(VERSION) | .appVersion = strenv(VERSION)' /dev/stdin < deploy/helm/kafka-operator/Chart.yaml
	yq eval -i '.version = strenv(DOCS_VERSION) | .prerelease = strenv(IS_NIGHTLY) != ""' /dev/stdin < docs/antora.yml

config:
	if [ -d "deploy/config-spec/" ]; then\
		mkdir -p deploy/helm/kafka-operator/configs;\
		cp -r deploy/config-spec/* deploy/helm/kafka-operator/configs;\
	fi

crds:
	mkdir -p deploy/helm/kafka-operator/crds
	cargo run --bin stackable-kafka-operator -- crd | yq eval '.metadata.annotations["helm.sh/resource-policy"]="keep"' - > deploy/helm/kafka-operator/crds/crds.yaml
